...
export const submissionsTable = pgTable("submissions", {
  ...
  /** 0–100 raw process-forensics suspicion score. Null when stream too sparse. */
  processScore: integer("process_score"),
  /** "human" | "mixed" | "likelyAI" — derived from processScore thresholds. */
  processClass: text("process_class"),
  /**
   * Computed feature dict, plus baseline-comparison metadata (double-underscore
   * keys: __baselineAdjustedScore, __baselineDeviation, __baselineSnapshot,
   * __baselineN). The admin renderer ignores __ keys in the feature loop.
   */
  processFeatures: jsonb("process_features"),
  /** Human-readable suspicion flags surfaced for admin review. */
